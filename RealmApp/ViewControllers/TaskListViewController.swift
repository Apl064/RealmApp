...
class TaskListViewController: UITableViewController {
    ...
    var sort: Int {
        sortSegment.selectedSegmentIndex
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if taskList.tasks.count == taskList.tasks.filter("isComplete = true").count
            && taskList.tasks.count > 0 {
            content.secondaryText = "Ok"
        } else {
    ...
    @IBAction func sortingList(_ sender: UISegmentedControl) {
        sorting(with: sender.selectedSegmentIndex)
        tableView.reloadData()
    }
    
    private func sorting(with index: Int) {
        self.taskLists = index == 0 ?
